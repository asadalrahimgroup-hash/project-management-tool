...
let valuesSql = tasksData.map((t, idx) => {
  return \          ('tsk-pvis-\', '\', 'Completed task: \', 'Done', 'High', '\', '\', 'proj-pvis', 'usr-pm-asad', 'usr-pm-asad', 'usr-pm-asad', '\ 23:59:59')\;
}).join(',\\n');

const fullSql = \      // Seed PVIS Tasks
      await client.query(\\\
...
// Replace the old tasks block
const regex = /\\/\\/ Seed PVIS Tasks[\\s\\S]*?ON CONFLICT \\(id\\) DO NOTHING;\\s*\\\\);/;
if (regex.test(dbJs)) {
  dbJs = dbJs.replace(regex, fullSql);
  fs.writeFileSync(dbJsPath, dbJs);
  console.log('Successfully expanded PVIS tasks to ' + tasksData.length + ' tasks!');
} else {
  console.log('Could not find the PVIS tasks block to replace.');
}

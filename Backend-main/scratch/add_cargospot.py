...
# Replace project details in-place
...
for i, (name, date) in enumerate(tasks_data):
    ...
    values_sql.append(f"          ('tsk-pia-{i+1}', '{safe_name}', 'Completed task: {safe_name}', 'Done', 'High', '{date}', '{date}', 'proj-pia-cargospot', 'usr-pm-asad', 'usr-pm-asad', 'usr-pm-asad', '{date} 23:59:59')")

new_tasks_block = ",\n".join(values_sql)

# Regex to find the old PIA tasks block and replace it
# The old block looks like: ('tsk-pia-1', ... to ('tsk-pia-55', ...
pattern = r"\(\'tsk-pia-1\'.*?\(\'tsk-pia-\d+\'.*?\d{4}-\d{2}-\d{2} 23:59:59\'\)"
if re.search(pattern, db_js, re.DOTALL):
    db_js = re.sub(pattern, new_tasks_block, db_js, flags=re.DOTALL)
else:
    print("Could not find the tsk-pia tasks block to replace!")
    exit(1)

with open(db_js_path, 'w', encoding='utf-8') as f:
    f.write(db_js)

print(f"PIA CargoSpot tasks ({len(tasks_data)}) successfully injected and updated!")

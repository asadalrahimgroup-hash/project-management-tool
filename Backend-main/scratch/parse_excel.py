...
# 3. SQL for Projects and Program Projects
projects_sql = []
pp_sql = []
pm_sql = []
tasks_sql = []
...
for proj, pdata in projects_dict.items():
    ...
    for student in pdata['students']:
        ...
        pm_sql.append(f"          ('pm-{pm_counter}', '{pid}', '{uid}', 'Intern')")
        pm_counter += 1
...
target = 'console.log("o. QARC Database schema'
if target in db_js:
    # We replace the target with final_sql + target
    db_js = db_js.replace(target, final_sql + '\\n      ' + target)
